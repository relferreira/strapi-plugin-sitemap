...
module.exports = {
  ...
  createSitemapEntries: async () => {
    ...
    await Promise.all(
      Object.keys(config.contentTypes).map(async (contentType) => {
        let modelName;
        const contentTypeByName = Object.values(strapi.contentTypes).find(
          (strapiContentType) => strapiContentType.info.name === contentType
        );
        ...
        const hasDraftAndPublish =
          strapi.query(modelName).model.__schema__.options.draftAndPublish;
        ...
        const pageData = await module.exports.getSitemapPageData(
          contentType,
          pages,
          config
        );
  ...
  createSitemap: async (sitemapEntries) => {
    ...
    const allSitemapEntries =
      sitemapEntries || (await module.exports.createSitemapEntries());
